...
interface MuteFriendsTabProps {
  ...
  handleSelectItem: (id: string, type: "friendMutes") => void;
  handleMuteFriendsSelected: () => void;
  isLoading: boolean;
  loadingItems: string[];
  handleSelectAll: (type: "friendMutes", ids: string[]) => void;
  deletionProgress: {
    deleted: number;
    total: number;
  };
...
export function MuteFriendsTab({
  userFriends,
  isAuthenticated,
  muteFriend,
  selectedItems,
  handleSelectItem,
  handleMuteFriendsSelected,
  isLoading,
  loadingItems,
  handleSelectAll,
  deletionProgress,
  ...
  const selectionProgress =
    userFriends.length > 0
      ? (selectedItems.friendMutes.length / userFriends.length) * 100
      : 0;
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Mute Friends</CardTitle>
        <CardDescription>
          Mute DM notifications from friends. This requires an existing DM
          channel with them.
        </CardDescription>
      </CardHeader>
      <CardContent>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4">
                    <Checkbox
                      id={`friend-mute-${friend.user.id}`}
                      checked={selectedItems.friendMutes.includes(friend.user.id)}
                      onCheckedChange={() =>
                        handleSelectItem(friend.user.id, "friendMutes")
                      }
                    />
                  ...
                  <Button
                    onClick={() => muteFriend(friend.user.id)}
                    variant="destructive"
                    size="sm"
      ...
      <CardFooter className="flex items-center justify-between">
        <Button
          onClick={handleMuteFriendsSelected}
          variant="destructive"
          disabled={selectedItems.friendMutes.length === 0 || isLoading}
        >
          {isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
          Mute Selected ({selectedItems.friendMutes.length})
        </Button>
        <Progress
        ...
        >
          {selectedItems.friendMutes.length === userFriends.length
            ? "Deselect All"
            : "Select All"}
        </Button>
      </CardFooter>
